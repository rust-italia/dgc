...
fn remove_prefix(data: &'_ str) -> Result<&'_ str, String> {
    ...
    if &data[0..4] != "HC1:" {
        return Err(format!(
            "Invalid header. Expected 'HC1:', found {}",
            &data[0..3]
        ));
    }
...
fn decode_base45(data: &str) -> Result<Vec<u8>, String> {
    base45::decode(data).map_err(|e| format!("Cannot base45 decode the data: {:?}", e))
}
...
fn parse_cose_payload(data: &[u8]) -> Result<Vec<u8>, String> {
    let sign1 =
        COSESign1::from_bytes(data).map_err(|e| format!("Cannot read COSE message: {}", e))?;

    let payload = sign1
        .get_payload(None)
        .map_err(|e| format!("Cannot read COSESign1 payload: {}", e))?;

    Ok(payload)
}

fn cbor_to_json(data: &[u8]) -> Result<String, String> {
    let mut output: Vec<u8> = vec![];
    let mut deserializer = serde_cbor::Deserializer::from_slice(data);
    let mut serializer = serde_json::Serializer::new(&mut output);

    serde_transcode::transcode(&mut deserializer, &mut serializer)
        .map_err(|e| format!("Cannot convert CBOR to JSON: {}", e))?;
